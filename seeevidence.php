...
<?php
    ...
    $query = "SELECT DISTINCT idadoption FROM evidence;";
    $result = mysqli_query($conex, $query);

    $query2 = "SELECT COUNT(*) AS 'a' FROM evidence;";
    $existevidence = mysqli_query($conex, $query2);
    $numberevidence =mysqli_fetch_array($existevidence);
...
<?php if($numberevidence['a'] != 0){ while($fila=mysqli_fetch_array($result)){ ?>

        <?php $aux = $fila['idadoption'];?>
        <?php $query2 = "SELECT * FROM adoptions WHERE idadoption = '$aux';"; ?>
        <?php $result2 = mysqli_query($conex, $query2); ?>
        <?php $fila2=mysqli_fetch_array($result2); ?>
        ...
        <div class="d-flex text-muted pt-5">
            ...
            <p class="pb-3 mb-0 small lh-sm border-bottom">
                ...
                <?php $aux = $fila2['idadopter'];?>
                <?php $query3 = "SELECT * FROM users WHERE iduser = '$aux';"; ?>
                <?php $result3 = mysqli_query($conex, $query3); ?>
                <?php $fila3=mysqli_fetch_array($result3); ?>
                <?php echo"<br><label class='text-gray-dark'><strong>Usuario que adopto:  </strong>",$fila3['name'],"  ",$fila3['plastname'],"  ",$fila3['mlastname']," ";?>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                ...
                <?php $aux = $fila2['idgiver'];?>
                <?php $query3 = "SELECT * FROM users WHERE iduser = '$aux';"; ?>
                <?php $result3 = mysqli_query($conex, $query3); ?>
                <?php $fila3=mysqli_fetch_array($result3); ?>
                <?php echo"<br><label class='text-gray-dark'><strong>Usuario que dio en adopcion:  </strong>",$fila3['name'],"  ",$fila3['plastname'],"  ",$fila3['mlastname'],"";?>
            </p>
        </div>

<?php }}else{?>

        <br><br><br><br><h3 class="lead text-center  bg-danger p-3 text-white">No hay adopciones de las cuales halla evidencia</h3>
